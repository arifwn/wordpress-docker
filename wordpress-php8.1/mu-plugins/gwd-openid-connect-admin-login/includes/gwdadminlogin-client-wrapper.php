...
/**
 * GWDAdminLogin_Client_Wrapper class.
 *
 ...
 * @category Authentication
 */
class GWDAdminLogin_Client_Wrapper {
	 ...
	 * @var string
	 */
	private $cookie_token_refresh_key = 'gwd-admin-login-refresh';
	 ...
	 * @var string
	 */
	public $cookie_redirect_key = 'gwd-admin-login-redirect';
	 ...
	 *
	 * @param GWDAdminLogin_Client          $client   A plugin client object instance.
	 ...
	 */
	public function __construct( GWDAdminLogin_Client $client, GWDAdminLogin_Option_Settings $settings, GWDAdminLogin_Option_Logger $logger ) {
		$this->client = $client;
	 ...
	 *
	 * @param \GWDAdminLogin_Client          $client   The plugin client instance.
	 * @param \GWDAdminLogin_Option_Settings $settings The plugin settings instance.
	 * @param \GWDAdminLogin_Option_Logger   $logger   The plugin logger instance.
	 *
	 * @return \GWDAdminLogin_Client_Wrapper
	 */
	public static function register( GWDAdminLogin_Client $client, GWDAdminLogin_Option_Settings $settings, GWDAdminLogin_Option_Logger $logger ) {
		...
		add_filter( 'gwd-admin-login-alter-request', array( $client_wrapper, 'alter_request' ), 10, 3 );
		...
		if ( is_admin() ) {
			 ...
			 */
			add_action( 'wp_ajax_gwd-admin-oid-connect-authorize', array( $client_wrapper, 'authentication_request_callback' ) );
			add_action( 'wp_ajax_nopriv_gwd-admin-oid-connect-authorize', array( $client_wrapper, 'authentication_request_callback' ) );
		}

		if ( $settings->alternate_redirect_uri ) {
			// Provide an alternate route for authentication_request_callback.
			add_rewrite_rule( '^gwd-admin-oid-connect-authorize/?', 'index.php?gwd-admin-oid-connect-authorize=1', 'top' );
			add_rewrite_tag( '%gwd-admin-oid-connect-authorize%', '1' );
	...
	public function alternate_redirect_uri_parse_request( $query ) {
		if ( isset( $query->query_vars['gwd-admin-oid-connect-authorize'] ) &&
			 '1' === $query->query_vars['gwd-admin-oid-connect-authorize'] ) {
	...
	public function get_redirect_to() {
		...
		$redirect_url = apply_filters_deprecated(
			'gwd-admin-login-cookie-redirect-url',
			array( $redirect_url ),
			'3.8.2',
			'gwd-admin-login-client-redirect-to'
		);

		// This is the new hook to use with the transients version of redirection.
		return apply_filters( 'gwd-admin-login-client-redirect-to', $redirect_url );
	}
	...
	public function get_authentication_url( $atts = array() ) {
		...
		$this->logger->log( apply_filters( 'gwd-admin-login-auth-url', $url ), 'make_authentication_url' );
		return apply_filters( 'gwd-admin-login-auth-url', $url );
	}
	...
	public function ensure_tokens_still_fresh() {
		...
		if ( ! $refresh_token || ( $refresh_expires && $current_time > $refresh_expires ) ) {
			if ( isset( $_SERVER['REQUEST_URI'] ) ) {
				do_action( 'gwd-admin-login-session-expired', wp_get_current_user(), esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ) );
				wp_logout();

				if ( $this->settings->redirect_on_logout ) {
					$this->error_redirect( new WP_Error( 'access-token-expired', __( 'Session expired. Please login again.', 'gwd-admin-login' ) ) );
				}
		...
		update_user_meta( $user_id, 'gwd-admin-login-last-token-response', $token_response );
	...
	public function get_end_session_logout_redirect_url( $redirect_url, $requested_redirect_to, $user ) {
		...
		$token_response = $user->get( 'gwd-admin-login-last-token-response' );
		if ( ! $token_response ) {
		...
		$claim = $user->get( 'gwd-admin-login-last-id-token-claim' );
	...
	public function authentication_request_callback() {
		...
		$token_response = apply_filters( 'gwd-admin-login-modify-token-response-before-validation', $token_response );
		...
		// Allow for other plugins to alter data before validation.
		$id_token_claim = apply_filters( 'gwd-admin-login-modify-id-token-claim-before-validation', $id_token_claim );
		...
		if ( ! $user ) {
			...
			if ( $this->settings->link_existing_users || $this->settings->create_if_does_not_exist ) {
				...
				$this->error_redirect( new WP_Error( 'identity-not-map-existing-user', __( 'User identity is not linked to an existing WordPress user.', 'gwd-admin-login' ), $user_claim ) );
			}
		}
		...
		do_action( 'gwd-admin-login-user-logged-in', $user );
		...
		$state_object = get_transient( 'gwd-admin-login-state--' . $state );
		...
		if ( $this->settings->redirect_user_back ) {
			do_action( 'gwd-admin-login-redirect-user-back', $redirect_url, $user );
		}
	...
	public function validate_user( $user ) {
		...
		if ( ! is_a( $user, 'WP_User' ) || ! $user->exists() ) {
			return new WP_Error( 'invalid-user', __( 'Invalid user.', 'gwd-admin-login' ), $user );
		}
	...
	public function refresh_user_claim( $user, $token_response ) {
		...
		update_user_meta( $user->ID, 'gwd-admin-login-last-token-response', $token_response );
		update_user_meta( $user->ID, 'gwd-admin-login-last-id-token-claim', $id_token_claim );
		update_user_meta( $user->ID, 'gwd-admin-login-last-user-claim', $user_claim );
	...
	public function login_user( $user, $token_response, $id_token_claim, $user_claim, $subject_identity ) {
		...
		do_action( 'gwd-admin-login-update-user-using-current-claim', $user, $user_claim );
	...
	public function get_user_by_identity( $subject_identity ) {
		// Look for user by their gwd-admin-login-subject-identity value.
		$user_query = new WP_User_Query(
			array(
				'meta_query' => array(
					array(
						'key'   => 'gwd-admin-login-subject-identity',
	...
	private function get_username_from_claim( $user_claim ) {
		...
		if ( empty( $desired_username ) ) {
			...
			return new WP_Error( 'no-username', __( 'No appropriate username found.', 'gwd-admin-login' ), $user_claim );
		}
		...
		if ( empty( $sanitized_username ) ) {
			...
			return new WP_Error( 'username-sanitization-failed', sprintf( __( 'Username %1$s could not be sanitized.', 'gwd-admin-login' ), $desired_username ), $desired_username );
		}
	...
	private function get_nickname_from_claim( $user_claim ) {
		...
		if ( empty( $desired_nickname ) ) {
			...
			return new WP_Error( 'no-nickname', sprintf( __( 'No nickname found in user claim using key: %1$s.', 'gwd-admin-login' ), $this->settings->nickname_key ), $this->settings->nickname_key );
		}
				...
				if ( ! $this->get_claim( $key, $user_claim, $info ) ) {
					if ( $error_on_missing_key ) {
						return new WP_Error(
							...
							__( 'User claim incomplete.', 'gwd-admin-login' ),
							array(
	...
	public function create_new_user( $subject_identity, $user_claim ) {
		$user_claim = apply_filters( 'gwd-admin-login-alter-user-claim', $user_claim );
		...
		if ( $values_missing && isset( $user_claim['access_token'] ) && ! empty( $this->settings->endpoint_userinfo ) ) {
			...
			if ( is_wp_error( $user_claim_result ) ) {
				return new WP_Error( 'bad-user-claim-result', __( 'Bad user claim result.', 'gwd-admin-login' ), $user_claim_result );
			}
		 ...
		 */
		$create_user = apply_filters( 'gwd-admin-login-user-creation-test', $this->settings->create_if_does_not_exist, $user_claim );

		if ( ! $create_user ) {
			return new WP_Error( 'cannot-authorize', __( 'Can not authorize.', 'gwd-admin-login' ), $create_user );
		}
		...
		$user_data = apply_filters( 'gwd-admin-login-alter-user-data', $user_data, $user_claim );
		...
		if ( is_wp_error( $uid ) ) {
			return new WP_Error( 'failed-user-creation', __( 'Failed user creation.', 'gwd-admin-login' ), $uid );
		}
		...
		add_user_meta( $user->ID, 'gwd-admin-login-subject-identity', (string) $subject_identity, true );

		// Log the results.
		$this->logger->log( "New user created: {$user->user_login} ($uid)", 'success' );

		// Allow plugins / themes to take action on new user creation.
		do_action( 'gwd-admin-login-user-create', $user, $user_claim );
	...
	public function update_existing_user( $uid, $subject_identity ) {
		...
		update_user_meta( $uid, 'gwd-admin-login-subject-identity', strval( $subject_identity ) );

		// Allow plugins / themes to take action on user update.
		do_action( 'gwd-admin-login-user-update', $uid );
