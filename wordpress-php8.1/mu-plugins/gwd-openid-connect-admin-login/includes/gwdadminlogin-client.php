 ...
 * @category Authentication
 */
class GWDAdminLogin_Client {

	/**
	 * The OIDC/oAuth client ID.
	 *
	 * @see GWDAdminLogin_Option_Settings::client_id
	 *
	 * @var string
	 */
	private $client_id;
	 ...
	 *
	 * @see GWDAdminLogin_Option_Settings::client_secret
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see GWDAdminLogin_Option_Settings::scope
	 *
	 * @var string
	 */
	private $scope;
	 ...
	 *
	 * @see GWDAdminLogin_Option_Settings::endpoint_login
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see GWDAdminLogin_Option_Settings::endpoint_userinfo
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see GWDAdminLogin_Option_Settings::endpoint_token
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see GWDAdminLogin_Option_Settings::redirect_uri
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see GWDAdminLogin_Option_Settings::acr_values
	 *
	 * @var string
	 */
	private $acr_values;
	 ...
	 *
	 * @see GWDAdminLogin_Option_Settings::state_time_limit
	 *
	 * @var int
	 */
	 ...
	 * Client constructor.
	 *
	 * @param string                               $client_id         @see GWDAdminLogin_Option_Settings::client_id for description.
	 * @param string                               $client_secret     @see GWDAdminLogin_Option_Settings::client_secret for description.
	 * @param string                               $scope             @see GWDAdminLogin_Option_Settings::scope for description.
	 * @param string                               $endpoint_login    @see GWDAdminLogin_Option_Settings::endpoint_login for description.
	 * @param string                               $endpoint_userinfo @see GWDAdminLogin_Option_Settings::endpoint_userinfo for description.
	 * @param string                               $endpoint_token    @see GWDAdminLogin_Option_Settings::endpoint_token for description.
	 * @param string                               $redirect_uri      @see GWDAdminLogin_Option_Settings::redirect_uri for description.
	 * @param string                               $acr_values        @see GWDAdminLogin_Option_Settings::acr_values for description.
	 * @param int                                  $state_time_limit  @see GWDAdminLogin_Option_Settings::state_time_limit for description.
	 * @param GWDAdminLogin_Option_Logger $logger            The plugin logging object instance.
	 */
	...
	public function validate_authentication_request( $request ) {
		...
		if ( ! isset( $request['state'] ) ) {
			do_action( 'gwd-admin-login-no-state-provided' );
			return new WP_Error( 'missing-state', __( 'Missing state.', 'gwd-admin-login' ), $request );
		}

		if ( ! $this->check_state( $request['state'] ) ) {
			return new WP_Error( 'invalid-state', __( 'Invalid state.', 'gwd-admin-login' ), $request );
		}
	...
	public function get_authentication_code( $request ) {
		if ( ! isset( $request['code'] ) ) {
			return new WP_Error( 'missing-authentication-code', __( 'Missing authentication code.', 'gwd-admin-login' ), $request );
		}
	...
	public function request_authentication_token( $code ) {
		...
		$request = apply_filters( 'gwd-admin-login-alter-request', $request, 'get-authentication-token' );
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'request_authentication_token', __( 'Request for authentication token failed.', 'gwd-admin-login' ) );
		}
	...
	public function request_new_tokens( $refresh_token ) {
		...
		// Allow modifications to the request.
		$request = apply_filters( 'gwd-admin-login-alter-request', $request, 'refresh-token' );
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'refresh_token', __( 'Refresh token failed.', 'gwd-admin-login' ) );
		}
	...
	public function get_token_response( $token_result ) {
		if ( ! isset( $token_result['body'] ) ) {
			return new WP_Error( 'missing-token-body', __( 'Missing token body.', 'gwd-admin-login' ), $token_result );
		}
		...
		if ( is_null( $token_response ) ) {
			return new WP_Error( 'invalid-token', __( 'Invalid token.', 'gwd-admin-login' ), $token_result );
		}
	...
	public function request_userinfo( $access_token ) {
		// Allow modifications to the request.
		$request = apply_filters( 'gwd-admin-login-alter-request', array(), 'get-userinfo' );
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'request_userinfo', __( 'Request for userinfo failed.', 'gwd-admin-login' ) );
		}
	...
	public function new_state( $redirect_to ) {
		...
		set_transient( 'gwd-admin-login-state--' . $state, $state_value, $this->state_time_limit );
	...
	public function check_state( $state ) {
		...
		if ( ! get_option( '_transient_gwd-admin-login-state--' . $state ) ) {
			do_action( 'gwd-admin-login-state-not-found', $state );
			$state_found = false;
		}

		$valid = get_transient( 'gwd-admin-login-state--' . $state );

		if ( ! $valid && $state_found ) {
			do_action( 'gwd-admin-login-state-expired', $state );
		}
	...
	public function get_authentication_state( $request ) {
		if ( ! isset( $request['state'] ) ) {
			return new WP_Error( 'missing-authentication-state', __( 'Missing authentication state.', 'gwd-admin-login' ), $request );
		}
	...
	public function get_id_token_claim( $token_response ) {
		...
		if ( ! isset( $token_response['id_token'] ) ) {
			return new WP_Error( 'no-identity-token', __( 'No identity token.', 'gwd-admin-login' ), $token_response );
		}
		...
		if ( ! isset( $tmp[1] ) ) {
			return new WP_Error( 'missing-identity-token', __( 'Missing identity token.', 'gwd-admin-login' ), $token_response );
		}
	...
	public function validate_id_token_claim( $id_token_claim ) {
		if ( ! is_array( $id_token_claim ) ) {
			return new WP_Error( 'bad-id-token-claim', __( 'Bad ID token claim.', 'gwd-admin-login' ), $id_token_claim );
		}

		// Validate the identification data and it's value.
		if ( ! isset( $id_token_claim['sub'] ) || empty( $id_token_claim['sub'] ) ) {
			return new WP_Error( 'no-subject-identity', __( 'No subject identity.', 'gwd-admin-login' ), $id_token_claim );
		}
		...
		if ( ! empty( $this->acr_values ) && isset( $id_token_claim['acr'] ) ) {
			if ( $this->acr_values != $id_token_claim['acr'] ) {
				return new WP_Error( 'no-match-acr', __( 'No matching acr values.', 'gwd-admin-login' ), $id_token_claim );
			}
		}
	...
	public function get_user_claim( $token_response ) {
		...
		if ( is_wp_error( $user_claim_result ) || ! isset( $user_claim_result['body'] ) ) {
			return new WP_Error( 'bad-claim', __( 'Bad user claim.', 'gwd-admin-login' ), $user_claim_result );
		}
	...
	public function validate_user_claim( $user_claim, $id_token_claim ) {
		...
		if ( ! is_array( $user_claim ) ) {
			return new WP_Error( 'invalid-user-claim', __( 'Invalid user claim.', 'gwd-admin-login' ), $user_claim );
		}

		// Allow for errors from the IDP.
		if ( isset( $user_claim['error'] ) ) {
			$message = __( 'Error from the IDP.', 'gwd-admin-login' );
		...
		if ( $id_token_claim['sub'] !== $user_claim['sub'] ) {
			return new WP_Error( 'incorrect-user-claim', __( 'Incorrect user claim.', 'gwd-admin-login' ), func_get_args() );
		}

		// Allow for other plugins to alter the login success.
		$login_user = apply_filters( 'gwd-admin-login-user-login-test', true, $user_claim );

		if ( ! $login_user ) {
			return new WP_Error( 'unauthorized', __( 'Unauthorized access.', 'gwd-admin-login' ), $login_user );
		}
